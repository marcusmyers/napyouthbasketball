...
class LeagueResourceTest extends TestCase
{
    ...
    public function test_team_can_be_retrieved_with_correct_resource_elements()
    {
    	...
    	$response = $this->actingAs($this->user)->get('/nova-api/leagues/'.$league->id);
    ...
    public function test_league_has_correct_validation_on_create()
    {
        ...
        $response = $this->actingAs($this->user)->post('/nova-api/leagues/', $league->toArray());
        $response->assertStatus(201);
    }

    public function test_name_is_required_on_create()
    {
        $response = $this->actingAs($this->user)->post('/nova-api/leagues/', ['name'=>null]);
